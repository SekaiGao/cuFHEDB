...
void healthcare_query3(const std::string &filePath) {
    std::cout << "Healthcare Query3 Test: "<< std::endl;
    std::cout << "--------------------------------------------------------"<< std::endl;
    ...
    */
    ...
    for (size_t i = 0; i < rows; i++) {
        ...
        cuFHEDB::less_than(cr.bill_ciphers[i], cr.bill_predicate, 2, filter_res0[i], ek, stream_id);
    ...
    for (size_t i = 0; i < rows; i++)
    {
		...
		cuFHEDB::equality(cr.gender_ciphers[i], cr.gender_male, pre_res, stream_id);
        cuFHEDB::HomAND(filter_res1[i], pre_res, filter_res0[i], ek, stream_id);
        cuFHEDB::HomNOT(pre_res, pre_res);
        cuFHEDB::HomAND(filter_res2[i], pre_res, filter_res0[i], ek, stream_id);
