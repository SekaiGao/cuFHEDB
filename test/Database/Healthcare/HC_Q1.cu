...
void healthcare_query1(const std::string &filePath) {
    std::cout << "Healthcare Query1 Test: "<< std::endl;
    std::cout << "--------------------------------------------------------"<< std::endl;
    ...
    for (size_t i = 0; i < records.Age.size(); i++) {
        ...
        cuFHEDB::equality(cr.medical_condition_ciphers[i], cr.medical_condition_predicate, filter_res0[i],  stream_id);

        // Filtering by age
        cuFHEDB::greater_than(cr.age_ciphers[i], cr.age_predicate0, pre_res, stream_id);
        cuFHEDB::HomAND(filter_res0[i], pre_res, filter_res0[i], ek, stream_id);
        cuFHEDB::less_than(cr.age_ciphers[i], cr.age_predicate1, pre_res,  stream_id);
        cuFHEDB::HomAND(filter_res0[i], pre_res, filter_res0[i], ek, stream_id);

        // Filtering by Gender
        cuFHEDB::equality(cr.gender_ciphers[i], cr.gender_predicate, pre_res,  stream_id);
        cuFHEDB::HomAND(filter_res0[i], pre_res, filter_res0[i], ek, stream_id);

        // Filtering by test results
        cuFHEDB::equality(cr.test_results_ciphers[i], cr.test_results_predicate, pre_res,  stream_id);
        cuFHEDB::HomAND(filter_res0[i], pre_res, filter_res0[i], ek, stream_id);

        // Filtering by date of admission
        cuFHEDB::greater_than(cr.date_of_admission_ciphers[i], cr.date_predicate,cr.date_of_admission_ciphers[i].size(), pre_res, ek,  stream_id);
        cuFHEDB::HomAND(filter_res0[i], pre_res, filter_res0[i], ek, stream_id);
