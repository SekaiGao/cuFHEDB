...
void healthcare_query2(const std::string &filePath) {
    std::cout << "Healthcare Query2 Test: "<< std::endl;
    std::cout << "--------------------------------------------------------"<< std::endl;
    ...
    for (size_t i = 0; i < rows; i++)
    {
		...
		cuFHEDB::equality(cr.InsuranceProvider_ciphers[i][0], cr.pattern_predicate, filter_res0[i], stream_id);

        // word-wise matching for LIKE
        for (int j=1;j<cr.InsuranceProvider_ciphers[i].size();++j){
            cuFHEDB::equality(cr.InsuranceProvider_ciphers[i][j], cr.pattern_predicate, pre_res, stream_id);
            cuFHEDB::HomOR(filter_res0[i], pre_res, filter_res0[i], ek, stream_id);
        }
    }
