 ...
 *  AND Date_of_Admission BETWEEN '2023/12/25' AND '2024/04/01'
    */
    ...
    for (size_t i = 0; i < rows; ++i) {
        ...
        greater_than_tfhepp(cr.date_of_admission_ciphers[i], cr.date_predicate0, cr.date_of_admission_ciphers[i].size(), pre_res0, ek, sk);
        less_than_tfhepp(cr.date_of_admission_ciphers[i], cr.date_predicate1, cr.date_of_admission_ciphers[i].size(), pre_res1, ek, sk);
        TFHEpp::HomAND(pre_res0, pre_res1, pre_res0, ek);

        // Filtering by medical condition 
        equality_tfhepp(cr.medical_condition_ciphers[i], cr.medical_condition_predicate, pre_res1, sk);
        TFHEpp::HomAND(filter_res1[i], pre_res1, pre_res0, ek);
    }
    end = std::chrono::system_clock::now();
    eval_time = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
    ...
    for (size_t i = 0; i < rows; ++i) {
        ...
        for (int j = 0; j < admission_type_attr_size; ++j) {
            equality_tfhepp(cr.admission_type_ciphers[i], cr.admission_type_attr_cipher[j], pre_res1, sk);
            TFHEpp::HomAND(filter_res[j][i], pre_res1, filter_res1[i], ek);
        }
    }
    end = std::chrono::system_clock::now();
    eval_time += std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
	...
	for (int i=0; i < admission_type_attr_size; ++i) {
		for (int j = 0; j < admission_type_attr_size; ++j) {
			if (i!=j) {
                ...
                // DESC
				HEDB::less_than<aggP>(agg_res[i], agg_res[j], tmp_cres, agg_bits, ek, LOGIC);
