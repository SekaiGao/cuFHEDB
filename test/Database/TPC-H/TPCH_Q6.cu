...
{
    std::cout << "TPC-H Query6 Test: "<< std::endl;
    std::cout << "--------------------------------------------------------"<< std::endl;
    ...
    std::uniform_int_distribution<uint64_t> revenue_message(1000, 1050);
    ...
    for (size_t i = 0; i < num; i++)
    {
        ...
        cuFHEDB::greater_than(discount_ciphers[i], predicate3_cipher, discount_ciphers[i].size(), pre_res, ek,  stream_id);
        cuFHEDB::HomAND(filter_res[i], pre_res, filter_res[i], ek, stream_id);
        cuFHEDB::less_than(discount_ciphers[i], predicate4_cipher, discount_ciphers[i].size(), pre_res, ek,  stream_id);
        cuFHEDB::HomAND(filter_res[i], pre_res, filter_res[i], ek, stream_id);
        cuFHEDB::less_than(quantity_ciphers[i], predicate5_cipher, quantity_ciphers[i].size(), pre_res, ek,  stream_id);
        cuFHEDB::lift_and_and(filter_res[i], pre_res, filter_res[i], 29, ek,  stream_id);
    }
    ...
    std::cout << "Ciphertext query result: " << std::endl;
    std::cout << "revenue: " << cipher_agg_res << std::endl;
