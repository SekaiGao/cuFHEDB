...
{
    std::cout << "TPC-H Query12 Test: "<< std::endl;
    std::cout << "--------------------------------------------------------"<< std::endl;
    ...
    for (size_t i = 0; i < o_num; i++) {
        cuFHEDB::less_than(orderpriority_ciphers[i], predicate_upper_bound_cipher, order_res[i],  omp_get_thread_num());
    }
    ...
    for (size_t i = 0; i < l_num; i++) {
        ...
        cuFHEDB::less_than(commitdate_ciphers[i], receiptdate_rgsw_ciphers[i], commitdate_ciphers[i].size(), filter_res_mail[i], ek,  stream_id);
        cuFHEDB::less_than(shipdate_ciphers[i], commitdate_rgsw_ciphers[i], shipdate_ciphers[i].size(), pre_res, ek,  stream_id);
        cuFHEDB::HomAND(filter_res_mail[i], pre_res, filter_res_mail[i], ek, stream_id);
        cuFHEDB::greater_than(receiptdate_ciphers[i], predicate_date_cipher1, receiptdate_ciphers[i].size(), pre_res, ek,  stream_id);
        cuFHEDB::HomAND(filter_res_mail[i], pre_res, filter_res_mail[i], ek, stream_id);
        cuFHEDB::less_than(receiptdate_ciphers[i], predicate_date_cipher2, receiptdate_ciphers[i].size(), pre_res, ek,  stream_id);
        cuFHEDB::HomAND(filter_res_mail[i], pre_res, filter_res_mail[i], ek, stream_id);
        filter_res_ship[i] = filter_res_mail[i];
        cuFHEDB::equality(shipmode_ciphers[i], predicate_mail_cipher, pre_res, stream_id);
        cuFHEDB::HomAND(filter_res_mail[i], pre_res, filter_res_mail[i], ek, stream_id);

        cuFHEDB::equality(shipmode_ciphers[i], predicate_ship_cipher, pre_res, stream_id);
        cuFHEDB::HomAND(filter_res_ship[i], pre_res, filter_res_ship[i], ek, stream_id);
    ...
    for (size_t i = 0; i < l_num; i++) {
        for (size_t j = 0; j < o_num; ++j) {
            ...
            cuFHEDB::equality(l_orderkey_ciphers[i], o_orderkey_ciphers[j], l_orderkey_ciphers[i].size(), pre_res, ek,  stream_id);
            cuFHEDB::HomAND(joined_res_mail[i * o_num + j], pre_res, filter_res_mail[i], ek, stream_id);
            cuFHEDB::HomAND(joined_res_ship[i * o_num + j], pre_res, filter_res_ship[i], ek, stream_id);

            cuFHEDB::lift_and_and(joined_res_mail[i * o_num + j], order_res[j], joined_res_mail[i * o_num + j], 29, ek,  stream_id);
            cuFHEDB::lift_and_and(joined_res_ship[i * o_num + j], order_res[j], joined_res_ship[i * o_num + j], 29, ek,  stream_id);
        }
    }
    ...
    for (size_t i = 0; i < l_num; i++) {
        ...
        // cuFHEDB::lift_and_and(filter_res_ship[i], filter_res_ship[i], pre_res_ship[i], agg_scale_bits, ek,  stream_id);
        // cuFHEDB::lift_and_and(filter_res_mail[i], filter_res_mail[i], pre_res_mail[i], agg_scale_bits, ek,  stream_id);
        // cuFHEDB::lift_and_and(count_ship_order[i], count_ship_order[i], pre_res_ship_order[i], agg_scale_bits, ek,  stream_id);
        // cuFHEDB::lift_and_and(count_mail_order[i], count_mail_order[i], pre_res_mail_order[i], agg_scale_bits, ek,  stream_id);
    }
