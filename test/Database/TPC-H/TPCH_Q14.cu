...
{
    std::cout << "TPC-H Query14 Test: "<< std::endl;
    std::cout << "--------------------------------------------------------"<< std::endl;
    ...
    for (size_t i = 0; i < p_num; i++)
    {
        ...
        TLWELvl1 pre_res;
        cuFHEDB::equality(ptype_ciphers[i][0], like_type_cipher, ptype_res[i],  stream_id);

        // word-wise matching for LIKE
        for (int j=1;j<max_like_len;++j){
            cuFHEDB::equality(ptype_ciphers[i][j], like_type_cipher, pre_res,  stream_id);
            cuFHEDB::HomOR(ptype_res[i], pre_res, ptype_res[i], ek, stream_id);
        }
    }
    ...
    for (size_t i = 0; i < l_num; i++)
    {
        ...
        cuFHEDB::greater_than(shipdate_ciphers[i], predicate1_cipher, shipdate_ciphers[i].size(), filter_res[i], ek,  stream_id);
        cuFHEDB::less_than(shipdate_ciphers[i], predicate2_cipher, shipdate_ciphers[i].size(), pre_res, ek,  stream_id);
        cuFHEDB::HomAND(filter_res[i], pre_res, filter_res[i], ek, stream_id);
    }
    end = std::chrono::system_clock::now();
    stept = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
    ...
    for (size_t i = 0; i < l_num; i++) {
        for(size_t j=0;j<p_num;++j) {
            uint32_t stream_id = omp_get_thread_num();
            TLWELvl1 pre_res;
            cuFHEDB::equality(l_pkey_ciphers[i], p_pkey_ciphers[j], l_pkey_ciphers[i].size(), pre_res, ek,  stream_id);
            cuFHEDB::HomAND(pre_res, pre_res, filter_res[i], ek, stream_id);
            //cuFHEDB::HomAND(joined_res[i*p_num+j], pre_res, ptype_res[j], ek, stream_id);
            cuFHEDB::lift_and_and(pre_res, ptype_res[j], joined_res[i*p_num+j], 29, ek,  stream_id);
        }
    }
    end = std::chrono::system_clock::now();
    stept = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
    ...
    for (size_t i = 0; i < l_num; i++)
    {
        ...
        for(size_t j=0;j<p_num;++j) {
            ...
            //cuFHEDB::HomOR(filter_case_res[i], filter_case_res[i], joined_res[i*p_num+j], ek, stream_id);
        }
        // rescale
        //cuFHEDB::lift_and_and(filter_case_res[i], filter_case_res[i], filter_case_res[i], 29, ek,  stream_id);
        cuFHEDB::lift_and_and(filter_res[i], filter_res[i], filter_res[i], 29, ek,  stream_id);
    }
