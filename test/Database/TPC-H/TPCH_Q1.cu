...
#define AGGREGATE false

size_t num = 1 << 10;

void tpch_query1(size_t num)
{
    std::cout << "TPC-H Query1 Test: "<< std::endl;
    std::cout << "--------------------------------------------------------"<< std::endl;
    ...
    for (size_t i = 0; i < num; i++)
    {
        ...
        cuFHEDB::less_than(ship_data_ciphers[i], shipdate_predicate, 2, filter_res_YY[i], ek,  stream_id);
        cuFHEDB::equality(returnflag_ciphers[i], returnflag_predicate_Y, pre_res_YY,  stream_id);
        cuFHEDB::HomAND(filter_res_YY[i], pre_res_YY, filter_res_YY[i], ek, stream_id);
        cuFHEDB::equality(linestatus_ciphers[i], linestatus_predicate_Y, pre_res_YY,  stream_id);
        cuFHEDB::lift_and_and(filter_res_YY[i], pre_res_YY, filter_res_YY[i], 29, ek,  stream_id);

        // returnflag = Y, linestatus = N
        cuFHEDB::less_than(ship_data_ciphers[i], shipdate_predicate, 2, filter_res_YN[i], ek,  stream_id);
        cuFHEDB::equality(returnflag_ciphers[i], returnflag_predicate_Y, pre_res_YN,  stream_id);
        cuFHEDB::HomAND(filter_res_YN[i], pre_res_YN, filter_res_YN[i], ek, stream_id);
        cuFHEDB::equality(linestatus_ciphers[i], linestatus_predicate_N, pre_res_YN,  stream_id);
        cuFHEDB::lift_and_and(filter_res_YN[i], pre_res_YN, filter_res_YN[i], 29, ek,  stream_id);

        // returnflag = N, linestatus = Y
        cuFHEDB::less_than(ship_data_ciphers[i], shipdate_predicate, 2, filter_res_NY[i], ek,  stream_id);
        cuFHEDB::equality(returnflag_ciphers[i], returnflag_predicate_N, pre_res_NY,  stream_id);
        cuFHEDB::HomAND(filter_res_NY[i], pre_res_NY, filter_res_NY[i], ek, stream_id);
        cuFHEDB::equality(linestatus_ciphers[i], linestatus_predicate_Y, pre_res_NY,  stream_id);
        cuFHEDB::lift_and_and(filter_res_NY[i], pre_res_NY, filter_res_NY[i], 29, ek,  stream_id);

        // returnflag = N, linestatus = N
        cuFHEDB::less_than(ship_data_ciphers[i], shipdate_predicate, 2, filter_res_NN[i], ek,  stream_id);
        cuFHEDB::equality(returnflag_ciphers[i], returnflag_predicate_N, pre_res_NN,  stream_id);
        cuFHEDB::HomAND(filter_res_NN[i], pre_res_NN, filter_res_NN[i], ek, stream_id);
        cuFHEDB::equality(linestatus_ciphers[i], linestatus_predicate_N, pre_res_NN,  stream_id);
        cuFHEDB::lift_and_and(filter_res_NN[i], pre_res_NN, filter_res_NN[i], 29, ek,  stream_id);
