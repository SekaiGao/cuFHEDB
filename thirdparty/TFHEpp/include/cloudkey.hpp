...
struct EvalKey {
    ...
    void emplacebkntt(const SecretKey& sk)
    {
        if constexpr (std::is_same_v<P, lvl01param>) {
            ...
            bknttlvlh1 = std::make_unique<BootstrappingKeyNTT<lvlh1param>>();
            ...
            bknttlvlh2 = std::make_unique<BootstrappingKeyNTT<lvlh2param>>();
    ...
    void emplaceiksk(const SecretKey& sk)
    {
        if constexpr (std::is_same_v<P, lvl10param>) {
            iksklvl10 = std::unique_ptr<KeySwitchingKey<lvl10param>>(
                new  KeySwitchingKey<lvl10param>());
            ...
            iksklvl1h = std::unique_ptr<KeySwitchingKey<lvl1hparam>>(
                new  KeySwitchingKey<lvl1hparam>());
            ...
            iksklvl22 = std::unique_ptr<KeySwitchingKey<lvl22param>>(new  KeySwitchingKey<lvl22param>()); 
            ...
            iksklvl31 = std::unique_ptr<KeySwitchingKey<lvl31param>>(
                new  KeySwitchingKey<lvl31param>());
    ...
    {
        if constexpr (std::is_same_v<P, lvl21param>) {
            subprivksklvl21[key] = std::make_unique<SubsetPrivateKeySwitchingKey<lvl21param>>();
...
template <class P>
using KBKElementFFT = std::array<TFHEpp::TRGSWFFT<typename P::targetP>, 3>;

template <class P>
using BKElementFFT = std::array<TFHEpp::TRGSWFFT<typename P::targetP>, 1>;

template <class P>
using KBKFFT = std::array<KBKElementFFT<P>, P::domainP::k * P::domainP::n / 2>;

template <class P>
using BKFFT = std::array<BKElementFFT<P>, P::domainP::k * P::domainP::n>;


template <class P>
void BKfftgen(BKFFT<P> &bkfft,
              const TFHEpp::Key<typename P::domainP> &domainkey,
              const TFHEpp::Key<typename P::targetP> &targetkey) {
  
  #pragma omp parallel for
  for (int i = 0; i < P::domainP::k * P::domainP::n; i++) {
	TFHEpp::Polynomial<typename P::targetP> plainpoly = {};
    plainpoly[0] = domainkey[i] == 1;
    bkfft[i][0] = TFHEpp::trgswfftSymEncrypt<typename P::targetP>(plainpoly, targetkey);
  }
}

template <class P>
void KBKfftgen(KBKFFT<P> &bkfft,
              const TFHEpp::Key<typename P::domainP> &domainkey,
              const TFHEpp::Key<typename P::targetP> &targetkey) {

  #pragma omp parallel for
  for (int i = 0; i < P::domainP::k * P::domainP::n / 2; i++) {
    TFHEpp::Polynomial<typename P::targetP> plainpoly = {};
    plainpoly[0] = static_cast<int32_t>(domainkey[2 * i] * domainkey[2 * i + 1]);
    bkfft[i][0] = TFHEpp::trgswfftSymEncrypt<typename P::targetP>(plainpoly, P::targetP::α, targetkey);
    plainpoly[0] = static_cast<int32_t>(domainkey[2 * i] * (1 - domainkey[2 * i + 1]));
    bkfft[i][1] = TFHEpp::trgswfftSymEncrypt<typename P::targetP>(plainpoly, P::targetP::α, targetkey);
    plainpoly[0] = static_cast<int32_t>((1 - domainkey[2 * i]) * domainkey[2 * i + 1]);
    bkfft[i][2] = TFHEpp::trgswfftSymEncrypt<typename P::targetP>(plainpoly, P::targetP::α, targetkey);
  }
}

template <class P>
void BKfftgen(BKFFT<P> &bkfft, const TFHEpp::SecretKey &sk) {
  BKfftgen<P>(bkfft, sk.key.get<typename P::domainP>(), sk.key.get<typename P::targetP>());
}

template <class P>
void KBKfftgen(KBKFFT<P> &bkfft, const TFHEpp::SecretKey &sk) {
  KBKfftgen<P>(bkfft, sk.key.get<typename P::domainP>(), sk.key.get<typename P::targetP>());
}
