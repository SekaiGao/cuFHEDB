...
__device__ alignas(64) double tables_direct_d[2048];
__device__ alignas(64) double tables_reverse_d[2048];
__device__ alignas(64) double tables_direct_d64[4096];
__device__ alignas(64) double tables_reverse_d64[4096];

// Whether use ASM optimization
#define ASM false
...
__device__ inline void FFT2(double4 &re, double4 &im) {
  asm volatile("{\n\t"
               ...
               "mov.f64 %7, tmp;\n\t"     // im.w = tmp
               "}"
               : "+d"(re.x), "+d"(re.y), "+d"(re.z), "+d"(re.w), "+d"(im.x),
                 "+d"(im.y), "+d"(im.z), "+d"(im.w)
               :);
}
...
__device__ inline double4 negaDouble4(double4 re) {
  double4 tmp;
  tmp.x = -re.x;
  tmp.y = -re.y;
  tmp.z = -re.z;
...
// butterfly operation
#if ASM
__device__ inline void CplxFma(double4 &tcs, double4 &tsn, double4 &re0,
                               double4 &im0, double4 &re1, double4 &im1) {
  asm volatile("{\n\t"
               ".reg .f64 tmp, tmp0;\n\t"
               ...
               : "d"(tcs.x), "d"(tcs.y), "d"(tcs.z), "d"(tcs.w), "d"(tsn.x),
                 "d"(tsn.y), "d"(tsn.z), "d"(tsn.w));
}
#else
__device__ inline void CplxFma(double4 &tcs, double4 &tsn, double4 &re0,
                               double4 &im0, double4 &re1, double4 &im1) {
...
__device__ inline void InvCplxFma(double4 &tcs, double4 &tsn, double4 &re0,
                                  double4 &im0, double4 &re1, double4 &im1) {
  register double4 tmp0, tmp1;
...
#if ASM
__device__ inline void CplxMul(double4 &tcs, double4 &tsn, double4 &re1,
                               double4 &im1) {
  asm volatile("{\n\t"
               ".reg .f64 tmp, tmp0;\n\t"
